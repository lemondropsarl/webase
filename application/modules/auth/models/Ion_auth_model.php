...
class Ion_auth_model extends CI_Model
{
	...
	public function __construct()
	{
		...
		$this->lang->load(['ion_auth','ion_auth_acl']);
    ...
    public function add_permission_to_group($group_id = FALSE, $perm_id = FALSE, $value = 0)
    {
        ...
        if( $existing_group_permission )
            $this->db->replace($this->tables['groups_permissions'], $data);
        else
            $this->db->insert($this->tables['groups_permissions'], $data);
    ...
    public function remove_permission_from_group($group_id = FALSE, $perm_id = FALSE)
    {
        ...
        $this->db->delete($this->tables['groups_permissions'], array('group_id' => $group_id, 'perm_id' => $perm_id));
	...
	public function get_groups_permissions(){
		$query = $this->db->get($this->tables['groups_permissions']);
		$matrix = array();
